	...
	if ( !( empty ( $arrFlickrData ) ) )
	{
		...
		if ( true == is_array( $unkFlickrAllUrls ) )
		{
			...
			for ( $intIndex = 0; $intIndex < $intNumOfPicsDisplayed; $intIndex++ )
			{
				...
				{
					echo '
						<span class="flickrImage">
							<a href="'.$arrUsePhotoUrls[$intIndex][0].'"><img alt="photo" 
									src="'.$arrUsePhotoUrls[$intIndex][1].'"
									title="'.htmlentities($arrUsePhotoUrls[$intIndex][2]).'"/></a>
						</span>';
				}
				else
				{
...
function includeCSS()
{
	echo '<link rel="stylesheet" type="text/css" href="'.get_option('siteurl').'/wp-content/plugins/flickr-thumbnails-photostream/style.css"/>';
}
